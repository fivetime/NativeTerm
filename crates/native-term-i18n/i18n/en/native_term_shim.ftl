...
plink-connecting = [NativeTerm] Connecting to { $target } ({ $protocol }) with plink…
...
plink-missing = [NativeTerm] plink.exe wasn't found: install PuTTY, or put plink.exe next to NativeTerm.
plink-not-started = [NativeTerm] Could not start plink ({ $path }): { $error }
