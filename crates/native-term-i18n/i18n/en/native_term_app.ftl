...
wizard-sync-note = To use the same sessions on several PCs, sync ~/.ssh/config.d with a tool you trust (without private keys). The data folder can be moved here or in Settings; built-in sync comes later.
wizard-back = Back
wizard-next = Next
