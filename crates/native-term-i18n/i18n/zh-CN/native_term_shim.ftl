...
zmodem-cancelled-enter = 已取消（服务器端也已停止；按回车回到提示符）
...
zmodem-tmux-stopped = tmux 会话里用不了 rz/sz（tmux 会改动传输的数据），已停止。按回车回到提示符。
zmodem-no-picker-files = 这个桌面没有文件选择对话框（zenity 或 kdialog）：已改为打开"文件（SFTP）"窗口。按回车回到提示符。
zmodem-no-picker = 这个桌面没有文件选择对话框（zenity 或 kdialog）：请用"文件（SFTP）"（标签菜单）。按回车回到提示符。
preconnect-running = 连接前执行：{ $command }
